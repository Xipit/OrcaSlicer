...
void GLCanvas3D::_render_assemble_control()
{
    ...
    ImGui::AlignTextToFramePadding();
    float same_line_width = 0.0f;
    {
        const float text_height = imgui->calc_text_size(_L("part selection")).y;
        ...
        GLGizmoUtils::TooltipButton(imgui, *this, m_shortcuts_assembly_view, x, y);
        ImGui::SameLine();
        ...
        same_line_width = button_size.x * 1.8; // with an space size
        ImGui::SameLine(same_line_width);
        same_line_width = imgui->calc_text_size("|"sv).x + same_line_width + imgui->calc_text_size("  "sv).x;
