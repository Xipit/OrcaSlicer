...
void GLGizmoFuzzySkin::TooltipButton(float x, float y)
{
    auto get_shortcuts = [this]() -> std::vector<std::pair<wxString, wxString>> {
        switch (m_tool_type) {
            case ToolType::BRUSH:
                return m_cursor_type == TriangleSelector::POINTER
                    ? m_shortcuts_triangle
                    : m_shortcuts_brush;
                                    
            case ToolType::SMART_FILL:
                return m_shortcuts_smart_fill;
            default:
                return {};
        }
    };

    GLGizmoUtils::TooltipButton(m_imgui, m_parent, get_shortcuts(), x, y);
}
